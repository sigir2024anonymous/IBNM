...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, mlp=False, debug=False, args=None):
        ...
        if self.args.multi_q:
            ...
            self.register_buffer("queue_large", torch.randn(2, self.qMult, 128, K))
            ...
            self.register_buffer("queue_large", torch.randn(2, 128, K))
    ...
    def forward(self, im_q, im_k=None, isedge_q=None, isedge_k=None,
        ...
        if not im_q_rgb is None:
            ...
            q_phase_large = extra_outputs['q_proj']
            q_rgb_large = extra_outputs_rgb['q_proj']
        else:
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            if isinstance(k, tuple) or isinstance(k, list):  # handle the case features are returned too
                k_ = k[0]
                k_large = torch.mean(torch.mean(k[1], dim=3), dim=2)
        ...
        if not cluster_result is None:
            logits_cluster_phase, labels_cluster_phase = self.cluster_logit_getter(cluster_result, extra_outputs['q_proj'],
                                                                                   q_selector, sample_idx)
            logits_cluster_rgb, labels_cluster_rgb = self.cluster_logit_getter(cluster_result, extra_outputs_rgb['q_proj'],
                                                                               q_selector, sample_idx)
