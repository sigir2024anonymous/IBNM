...
def main_worker(gpu, args):
    ...
    if args.arch == 'resnet50':
        model = moco.builder.MoCo(
            ...
            args.dim, args.moco_k, args.moco_m, args.moco_t, args.mlp,
            args=args
        )
    elif args.arch == 'vit':
        ...
        model = moco.builder.MoCo(
            model.backbone,
            args.dim, args.moco_k, args.moco_m, args.moco_t, args.mlp,
            args=args,num_cluster=args.num_cluster, 
        )
        config.defrost()    
        config.MODEL.NUM_FEATURES = model.base_encoder.embed_dim    
        config.MODEL.NUM_CLASSES = args.num_cluster
        config.freeze()
    ...
    if args.arch == 'vit' or args.arch == 'moe':
        # vit optimizer    
...
def train(train_loader, model, criterion, optimizer, epoch, args, cluster_result):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # b = batch
        # b, _, di, isrc = b[0],b[1],b[2],b[3]  # throw away the class labels
        # if images is None:
        #     images = [[] for _ in range(len(b))]
        # for j in range(len(b)):
        #     images[j].append(b[j])
        # domain_label.append(i * torch.ones((b[0].shape[0]), dtype=torch.long))
        # domain_index.append(di)
        # img_src += isrc
