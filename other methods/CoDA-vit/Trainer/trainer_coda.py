    ...
    for batch_idx, (inputs_src1, _, domainIDs_src, indexes_src) in enumerate(source_loader):
        ...
        for i in range(len(args.cluster_num_list)):
            if args.in_domain:
                ...
                loss_iss1 += nn.KLDivLoss(reduction='batchmean')(logits_src1[i].softmax(1).log(), (logits_mem_src1 / tau).softmax(1).detach())

                logits_mem_tar1 = model.top_layer_tgt[i](model.lemniscate_t.memory[indexes_tar])
                loss_iss2 += nn.KLDivLoss(reduction='batchmean')(logits_tar1[i].softmax(1).log(), (logits_mem_tar1 / tau).softmax(1).detach())
...
def retrieval_precision_cal_coda(features_A, targets_A, features_B, targets_B, prec_nums=(50, 100, 200)):

    dists = cosine_similarity(features_A, features_B)

    res_A = []
    res_B = []
    prec_nums = [int(prec_nums[0]),int(prec_nums[1]),int(prec_nums[2])]
    for domain_id in ['A', 'B']:
        if domain_id == 'A':
            query_targets = targets_A
            gallery_targets = targets_B
...
def test_target(args, src_loader, tgt_loader, netF, prec_nums,path_for_save_features=None):
    netF.eval()
    ...
    res_A, res_B = retrieval_precision_cal_coda(features_query, gt_labels_query,features_gallery, gt_labels_gallery,prec_nums)
    #return map_t * 100
    return res_A, res_B
